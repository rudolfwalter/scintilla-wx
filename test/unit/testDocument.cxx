...
struct DocPlus {
	...
	DocPlus(Sci::string_view svInitial, int codePage) : document(DocumentOption::Default) {
	...
	Sci::Position FindNeedle(Sci::string_view needle, FindOption options, Sci::Position *length) {
	...
	Sci::Position FindNeedleReverse(Sci::string_view needle, FindOption options, Sci::Position *length) {
		assert(*length == static_cast<Sci::Position>(needle.length()));
		return document.FindText(document.Length(), 0, needle.data(), options, length);
	}

	Match FindString(Sci::Position minPos, Sci::Position maxPos, Sci::string_view needle, FindOption flags) {
	...
	std::string Substitute(Sci::string_view substituteText) {
...
void TimeTrace(Sci::string_view sv, const Catch::Timer &tikka) {
...
TEST_CASE("Document") {
	...
	SECTION("SearchInLatin") {
		...
		constexpr Sci::string_view finding = "b";
	...
	SECTION("SearchInBothSegments") {
		...
		constexpr Sci::string_view finding = "ab";
	...
	SECTION("InsensitiveSearchIn1252") {
		...
		doc.SetSBCSFoldings(foldings1252, Sci::size(foldings1252));

		// Search for upper-case AE
		Sci::string_view finding = "\xc6";
		Sci::Position lengthFinding = finding.length();
		Sci::Position location = doc.FindNeedle(finding, FindOption::None, &lengthFinding);
		REQUIRE(location == 3);
	...
	SECTION("Search2InLatin") {
		...
		constexpr Sci::string_view finding = "cd";
		Sci::Position lengthFinding = finding.length();
	...
	SECTION("SearchInUTF8") {
		...
		constexpr Sci::string_view findingUTF = "\xCE\x93";
	...
	SECTION("InsensitiveSearchInShiftJIS") {
		...
		location = doc.document.FindText(doc.document.Length(), 0, finding.data(), FindOption::None, &lengthFinding);
		REQUIRE(location == 1);
		constexpr Sci::string_view finding932 = "\xe9" "b";
		// Search forwards
	...
	SECTION("NextPosition Valid DBCS") {
		...
		// U+958B open
		// U+7DE8 arrange
		constexpr Sci::string_view japaneseText = "Open=\x8aJ\x82\xad(O)\x95\xd2-";
	...
	SECTION("RegexSearchAndSubstitution") {
		...
		constexpr Sci::string_view finding = R"(\d+(\w+))";
		constexpr Sci::string_view substituteText = R"(\t\1\n)";
		constexpr Sci::string_view longest = "\\w+";
	...
	SECTION("RegexAssertion") {
		...
		constexpr Sci::string_view findingBOL = "^";
		...
		constexpr Sci::string_view findingEOL = "$";
		...
		constexpr Sci::string_view findingBOW = "\\<";
		...
		constexpr Sci::string_view findingEOW = "\\>";
		...
		constexpr Sci::string_view findingEOWEOL = "\\>$";
		...
		#ifndef NO_CXX11_REGEX
		constexpr Sci::string_view findingWB = "\\b";
		...
		constexpr Sci::string_view findingNWB = "\\B";
	...
	SECTION("RESearchMovePositionOutsideCharUTF8") {
		...
		const Sci::Position docLength = doc.document.Length();
		constexpr Sci::string_view finding = R"([a-z](\w)\1)";
...
TEST_CASE("SafeSegment") {
	SECTION("Short") {
		...
		constexpr Sci::string_view text = "12 ";
	...
	SECTION("UTF-8") {
		...
		Sci::string_view text = "JapaneseJa";
		size_t length = doc.document.SafeSegment(text);
		REQUIRE(text[length - 1] == 'J');
		REQUIRE(text[length] == 'a');
	...
	SECTION("DBCS Shift-JIS") {
		...
		Sci::string_view text = "(IsBreakSpace(text[j]))";
		size_t length = doc.document.SafeSegment(text);
		REQUIRE(text[length - 1] == 'j');
		REQUIRE(text[length] == ']');
