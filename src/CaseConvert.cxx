...
constexpr Compat::string_view complexCaseConversions =
...
class CaseConverter final : public ICaseConverter {
	...
	struct CharacterConversion {
		...
		CharacterConversion(int character_, Compat::string_view conversion_) noexcept : character(character_) {
			assert(conversion_.length() <= maxConversionLength);
			try {
				// This can never fail as Compat::string_view::copy should only throw
...
public:
	...
	void Add(int character, Compat::string_view conversion_) {
...
Compat::string_view NextField(Compat::string_view &view) {
	const size_t separatorPosition = view.find_first_of('|');
	const Compat::string_view field = view.substr(0, separatorPosition);
	if (separatorPosition == Compat::string_view::npos) {
...
void CaseConverter::SetupConversions(CaseConversion conversion) {
	...
	Compat::string_view sComplex = complexCaseConversions;
	while (!sComplex.empty()) {
		const Compat::string_view originUTF8 = NextField(sComplex);
		const Compat::string_view foldedUTF8 = NextField(sComplex);
		const Compat::string_view upperUTF8 = NextField(sComplex);
		const Compat::string_view lowerUTF8 = NextField(sComplex);

		Compat::string_view converted;
		switch (conversion) {
