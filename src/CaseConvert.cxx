...
SCI_CONSTEXPR14 const Sci::string_view complexCaseConversions =
...
class CaseConverter final : public ICaseConverter {
	...
	struct CharacterConversion {
		...
		CharacterConversion(int character_, Sci::string_view conversion_) noexcept : character(character_) {
			assert(conversion_.length() <= maxConversionLength);
			try {
				// This can never fail as Sci::string_view::copy should only throw
...
public:
	...
	void Add(int character, Sci::string_view conversion_) {
...
Sci::string_view NextField(Sci::string_view &view) {
	const size_t separatorPosition = view.find_first_of('|');
	const Sci::string_view field = view.substr(0, separatorPosition);
	if (separatorPosition == Sci::string_view::npos) {
...
void CaseConverter::SetupConversions(CaseConversion conversion) {
	...
	Sci::string_view sComplex = complexCaseConversions;
	while (!sComplex.empty()) {
		const Sci::string_view originUTF8 = NextField(sComplex);
		const Sci::string_view foldedUTF8 = NextField(sComplex);
		const Sci::string_view upperUTF8 = NextField(sComplex);
		const Sci::string_view lowerUTF8 = NextField(sComplex);

		Sci::string_view converted;
		switch (conversion) {
...
CaseConverter *ConverterForConversion(CaseConversion conversion) {
	...
	assert(index < Sci::size(caseConvList));
