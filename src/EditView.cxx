...
int WidthStyledText(Surface *surface, const ViewStyle &vs, int styleOffset,
	...
	while (start < len) {
		...
		const Compat::string_view sv(text + start, endSegment - start + 1);
...
int WidestLineWidth(Surface *surface, const ViewStyle &vs, int styleOffset, const StyledText &st) {
	...
	while (start < st.length) {
		...
		if (st.multipleStyles) {
			...
			const Compat::string_view text(st.text + start, lenLine);
...
void DrawTextNoClipPhase(Surface *surface, PRectangle rc, const Style &style, XYPOSITION ybase,
	Compat::string_view text, DrawPhase phase) {
...
void DrawStyledText(Surface *surface, const ViewStyle &vs, int styleOffset, PRectangle rcText,
	...
	if (st.multipleStyles) {
		...
		while (i < length) {
			...
			const Compat::string_view text(st.text + start + i, end - i + 1);
		...
		DrawTextNoClipPhase(surface, rcText, vs.styles[style],
			...
			Compat::string_view(st.text + start, length), phase);
	}
}
...
void EditView::SetLayoutThreads(unsigned int threads) noexcept {
	maxLayoutThreads = Compat::clamp(threads, 1U, std::thread::hardware_concurrency());
}
			...
			if (ts.representation) {
				...
				if ((ts.length == 1) && (' ' == ll->chars[ts.start])) {
					...
					pCache->MeasureWidths(surface, vstyle, styleSegment, textUnicode,
						Compat::string_view(&ll->chars[ts.start], ts.length), positions, multiThreaded);
				}
			}
		} else if (vstyle.styles[styleSegment].invisibleRepresentation[0]) {
			const Compat::string_view text = vstyle.styles[styleSegment].invisibleRepresentation;
			XYPOSITION positionsRepr[Representation::maxLength + 1];
...
void EditView::UpdateBidiData(const EditModel &model, const ViewStyle &vstyle, LineLayout *ll) {
	if (model.BidirectionalEnabled()) {
		...
		for (int charsInLine = 0; charsInLine < ll->numCharsInLine; charsInLine++) {
			...
			const Representation *repr = model.reprs->RepresentationFromCharacter(Compat::string_view(&ll->chars[charsInLine], charWidth));
...
void DrawTextBlob(Surface *surface, const ViewStyle &vsDraw, PRectangle rcSegment,
	Compat::string_view text, ColourRGBA textBack, ColourRGBA textFore, bool fillBackground) {
	if (rcSegment.Empty())
		return;
	if (fillBackground) {
...
void EditView::DrawEOL(Surface *surface, const EditModel &model, const ViewStyle &vsDraw, const LineLayout *ll,
	...
	if (lastSubLine) {
		for (Sci::Position eolPos = ll->numCharsBeforeEOL; eolPos<ll->numCharsInLine;) {
			...
			char hexits[4] = "";
			Compat::string_view ctrlChar;
			Sci::Position widthBytes = 1;
			RepresentationAppearance appearance = RepresentationAppearance::Blob;
			const Representation *repr = model.reprs->RepresentationFromCharacter(Compat::string_view(&ll->chars[eolPos], ll->numCharsInLine - eolPos));
			if (repr) {
				// Representation of whole text
				widthBytes = ll->numCharsInLine - eolPos;
			} else {
				repr = model.reprs->RepresentationFromCharacter(Compat::string_view(&ll->chars[eolPos], 1));
			}
			if (repr) {
...
void EditView::DrawFoldDisplayText(Surface *surface, const EditModel &model, const ViewStyle &vsDraw, const LineLayout *ll,
	...
	PRectangle rcSegment = rcLine;
	const Compat::string_view foldDisplayText(text);
...
void EditView::DrawEOLAnnotationText(Surface *surface, const EditModel &model, const ViewStyle &vsDraw, const LineLayout *ll,
	...
	const Compat::string_view eolAnnotationText(stEOLAnnotation.text, stEOLAnnotation.length);
	...
	if (textFoldDisplay) {
		const Compat::string_view foldDisplayText(textFoldDisplay);
...
void DrawBlockCaret(Surface *surface, const EditModel &model, const ViewStyle &vsDraw, const LineLayout *ll,
	...
	const Compat::string_view text(&ll->chars[offsetFirstChar], numCharsToDraw);
		...
		if (rcLine.Intersects(horizontal)) {
			...
			if (ts.representation) {
				...
				if (vsDraw.styles[styleMain].visible) {
					const Compat::string_view text(&ll->chars[ts.start], i - ts.start + 1);
					if (phasesDraw != PhasesDraw::One) {
					...
					const Compat::string_view text = vsDraw.styles[styleMain].invisibleRepresentation;
  					if (phasesDraw != PhasesDraw::One) {
