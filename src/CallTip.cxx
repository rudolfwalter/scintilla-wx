...
int CallTip::DrawChunk(Surface *surface, int x, Sci::string_view sv,
	...
	for (const size_t endSeg : ends) {
		...
		if (IsArrowCharacter(sv[startSeg])) {
			...
			const Sci::string_view segText = sv.substr(startSeg, endSeg - startSeg);
...
int CallTip::PaintContents(Surface *surfaceWindow, bool draw) {
	...
	Sci::string_view remaining(val);
	int maxWidth = 0;
	size_t lineStart = 0;
	while (!remaining.empty()) {
		const Sci::string_view chunkVal = remaining.substr(0, remaining.find_first_of('\n'));
