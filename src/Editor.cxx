...
inline Message WithExtends(Message iMessage) noexcept {
	switch (iMessage) {
...
inline int NaturalDirection(Message iMessage) noexcept {
	switch (iMessage) {
	case Message::CharLeft:
	case Message::CharLeftExtend:
	case Message::CharLeftRectExtend:
...
inline bool IsRectExtend(Message iMessage, bool isRectMoveExtends) noexcept {
	switch (iMessage) {
	case Message::CharLeftRectExtend:
	case Message::CharRightRectExtend:
	case Message::HomeRectExtend:
...
sptr_t Editor::WndProc(Message iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case Message::SetCaretWidth:
		vs.caret.width = Sci::clamp(static_cast<int>(wParam), 0, 20);
		InvalidateStyleRedraw();
		break;
