...
protected:	// ScintillaBase subclass needs access to much of Editor
	...
	virtual void InsertCharacter(Compat::string_view sv, Scintilla::CharacterSource charSource);
	...
	Sci::Position ReplaceTarget(ReplaceType replaceType, Compat::string_view text);
	...
	virtual std::string UTF8FromEncoded(Compat::string_view encoded) const = 0;
	virtual std::string EncodedFromUTF8(Compat::string_view utf8) const = 0;
	virtual std::unique_ptr<Surface> CreateMeasurementSurface() const;
	virtual std::unique_ptr<Surface> CreateDrawingSurface(SurfaceID sid, Compat::optional<Scintilla::Technology> technologyOpt = {}) const;
	...
	static Compat::string_view ViewFromParams(Scintilla::sptr_t lParam, Scintilla::uptr_t wParam) noexcept {
		if (SPtrFromUPtr(wParam) == -1) {
			return Compat::string_view(CharPtrFromSPtr(lParam));
		}
		return Compat::string_view(CharPtrFromSPtr(lParam), wParam);
	}
	...
	static /*constexpr*/ Compat::optional<FoldLevel> OptionalFoldLevel(Scintilla::sptr_t lParam) {
		if (lParam >= 0) {
	...
	static Scintilla::sptr_t BytesResult(Scintilla::sptr_t lParam, Compat::string_view sv) noexcept;
...
public:
	...
	AutoSurface(SurfaceID sid, const Editor *ed, Compat::optional<Scintilla::Technology> technology = {}) :
